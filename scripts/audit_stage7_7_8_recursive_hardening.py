...
def _validate_phase9_boundary(failures: list[str]) -> dict[str, int]:
    ...
    if stage9.get("status") != "stage9_23_figure_legend_caption_audit_bound":
        failures.append("roadmap execution memory must record Stage 9 as stage9_23_figure_legend_caption_audit_bound")
    ...
    if current.get("active_stage") != "Stage 9.23 Figure legend and caption audit complete; editorial polish pass I not started":
        failures.append("roadmap active stage must record the Stage 9.23 figure-legend boundary")
    return {
