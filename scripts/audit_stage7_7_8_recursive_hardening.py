...
FORBIDDEN_STAGE9_DRAFT_FILES = set()
...
def _validate_phase9_boundary(failures: list[str]) -> dict[str, int]:
    ...
    if stage9.get("status") != "stage9_28_pi_review_packet_complete":
        failures.append("roadmap execution memory must record Stage 9 as stage9_28_pi_review_packet_complete")
    ...
    if "9.7" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.7 supplementary display-plan substage")
    ...
    if "9.11" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.11 Results drafting substage")
    if "9.12" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.12 Introduction literature-binding substage")
    if "9.13" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.13 Discussion interpretation-map substage")
    if "9.14" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.14 Discussion drafting substage")
    ...
    if "9.20" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.20 reference-library substage")
    if "9.21" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.21 cross-document consistency substage")
    ...
    if "9.25b" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.25b reader-surface hygiene substage")
    if "9.26" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.26 internal peer-review substage")
    if "9.27" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.27 submission-package substage")
    if "9.28" not in substage_ids:
        failures.append("Stage 9 execution memory must include the 9.28 PI-review substage")
    ...
    if substage_status.get("9.27") != "complete_submission_package_assembled":
        failures.append("Stage 9.27 must be marked complete_submission_package_assembled")
    if substage_status.get("9.28") != "complete_pi_review_packet":
        failures.append("Stage 9.28 must be marked complete_pi_review_packet")
    ...
    if current.get("active_stage") != "Stage 9.28 PI review packet complete; Stage 9 closure not started":
        failures.append("roadmap active stage must record the Stage 9.28 PI-review boundary")
    return {
