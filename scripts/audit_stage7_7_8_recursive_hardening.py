...
def _validate_phase9_boundary(failures: list[str]) -> dict[str, int]:
    ...
    if stage9.get("status") != "stage9_22_statistical_language_audit_bound":
        failures.append("roadmap execution memory must record Stage 9 as stage9_22_statistical_language_audit_bound")
    ...
    if current.get("active_stage") != "Stage 9.22 Statistical and quantitative language audit complete; figure legend and caption audit not started":
        failures.append("roadmap active stage must record the Stage 9.22 statistical-language boundary")
    return {
