...
def _validate_phase9_boundary(failures: list[str]) -> dict[str, int]:
    ...
    if stage9.get("status") != "stage9_26_internal_peer_review_bound":
        failures.append("roadmap execution memory must record Stage 9 as stage9_26_internal_peer_review_bound")
    ...
    if current.get("active_stage") != "Stage 9.26 Internal peer review simulation complete; submission package assembly not started":
        failures.append("roadmap active stage must record the Stage 9.26 internal peer-review boundary")
    return {
