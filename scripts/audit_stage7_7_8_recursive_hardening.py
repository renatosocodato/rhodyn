...
def _validate_phase9_boundary(failures: list[str]) -> dict[str, int]:
    ...
    if stage9.get("status") != "stage9_25b_reader_surface_hygiene_bound":
        failures.append("roadmap execution memory must record Stage 9 as stage9_25b_reader_surface_hygiene_bound")
    ...
    if current.get("active_stage") != "Stage 9.25b Reader-surface hygiene complete; internal peer review not started":
        failures.append("roadmap active stage must record the Stage 9.25b reader-surface hygiene boundary")
    return {
