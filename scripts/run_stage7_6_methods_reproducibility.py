...
def _roadmap_state_scan(root: Path) -> StepResult:
    ...
    if current.get("active_stage") != "Stage 9.23 Figure legend and caption audit complete; editorial polish pass I not started":
        failures.append("roadmap memory does not mark the Stage 9.23 figure-legend boundary as active")
    ...
    if stage9.get("status") != "stage9_23_figure_legend_caption_audit_bound":
        failures.append("Stage 9 is not marked stage9_23_figure_legend_caption_audit_bound")
    ...
    if "9.8" not in stage9_substage_ids:
    ...
    if "9.13" not in stage9_substage_ids:
        failures.append("Stage 9 does not serialize the 9.13 Discussion interpretation-map substage")
    if "9.14" not in stage9_substage_ids:
    ...
    if "9.17" not in stage9_substage_ids:
        failures.append("Stage 9 does not serialize the 9.17 availability assembly substage")
    if "9.18" not in stage9_substage_ids:
        failures.append("Stage 9 does not serialize the 9.18 Supplementary Methods substage")
    if "9.19" not in stage9_substage_ids:
        failures.append("Stage 9 does not serialize the 9.19 supplementary table/source-data substage")
    if "9.20" not in stage9_substage_ids:
    ...
    if "9.23" not in stage9_substage_ids:
    ...
    if stage9_substage_status.get("9.8") != "complete_section_contract_blueprint_registered":
    ...
    if stage9_substage_status.get("9.13") != "complete_discussion_interpretation_mapped":
        failures.append("Stage 9.13 is not marked complete_discussion_interpretation_mapped")
    if stage9_substage_status.get("9.14") != "complete_discussion_drafted":
    ...
    if stage9_substage_status.get("9.17") != "complete_availability_assembled":
        failures.append("Stage 9.17 is not marked complete_availability_assembled")
    if stage9_substage_status.get("9.18") != "complete_supplementary_methods_drafted":
        failures.append("Stage 9.18 is not marked complete_supplementary_methods_drafted")
    if stage9_substage_status.get("9.19") != "complete_supplementary_tables_bound":
        failures.append("Stage 9.19 is not marked complete_supplementary_tables_bound")
    if stage9_substage_status.get("9.20") != "complete_reference_library_bound":
    ...
    if stage9_substage_status.get("9.23") != "complete_figure_legend_caption_audit_bound":
