...
OUTPUTS = {
    ...
    "editorial_bar_rescue_audit": SUBMISSION / "nature_methods_editorial_bar_rescue_audit.md",
...
def _editorial_bar_rescue_audit() -> str:
    return """# Nature Methods editorial-bar rescue audit
...
def _readiness_checklist(checks: list[dict[str, Any]]) -> str:
    ...
    lines = [
        ...
        f"| Editorial-bar rescue audit | {'ready' if check_map.get('editorial_bar_rescue_audit_present') else 'blocked'} | `nature_methods_editorial_bar_rescue_audit.md` stress-tests novelty, benchmarking, generality, figure architecture, claims, and the final submit/delay decision against the Nature Methods Article bar. |",
...
def _audit(generated_utc: str, package_dir: Path) -> dict[str, Any]:
    ...
    checks = [
        ...
        {
            "name": "editorial_bar_rescue_audit_present",
            "passed": (package_dir / "nature_methods_editorial_bar_rescue_audit.md").exists()
            and "Nature Methods editorial-bar rescue audit" in (package_dir / "nature_methods_editorial_bar_rescue_audit.md").read_text(encoding="utf-8")
            and "Top Desk-Rejection Risks" in (package_dir / "nature_methods_editorial_bar_rescue_audit.md").read_text(encoding="utf-8")
            and "One-Sentence Nature Methods Advance" in (package_dir / "nature_methods_editorial_bar_rescue_audit.md").read_text(encoding="utf-8")
            and "Nature Methods Requirement Matrix" in (package_dir / "nature_methods_editorial_bar_rescue_audit.md").read_text(encoding="utf-8")
            and "Novelty Ledger" in (package_dir / "nature_methods_editorial_bar_rescue_audit.md").read_text(encoding="utf-8")
            and "Comparator Ledger" in (package_dir / "nature_methods_editorial_bar_rescue_audit.md").read_text(encoding="utf-8")
            and "Minimum Elevation Package" in (package_dir / "nature_methods_editorial_bar_rescue_audit.md").read_text(encoding="utf-8")
            and "Final Decision" in (package_dir / "nature_methods_editorial_bar_rescue_audit.md").read_text(encoding="utf-8")
            and "Submit after minor narrative elevation and completion of human upload actions" in (package_dir / "nature_methods_editorial_bar_rescue_audit.md").read_text(encoding="utf-8"),
            "detail": "Editorial-bar rescue audit stress-tests novelty, validation, generality, benchmarking, figure architecture, claim discipline, and final decision",
        },
        {
...
def _stage_outputs(generated_utc: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    _write_text(staging_submission / "nature_methods_editorial_bar_rescue_audit.md", _editorial_bar_rescue_audit())
...
def _update_stage9_memory(generated_utc: str, checks: list[dict[str, Any]]) -> None:
    ...
    memory["gate"] = [
        ...
        "The package contains reader-clean main text, Supplementary Information, code-for-review, cover-letter submission draft, final upload runbook, prior-art positioning matrix, validation breadth map, editor-objection response map, two-minute editor triage simulation, Nature Methods editorial-bar rescue audit, current Nature Methods policy preflight, reviewer/editor fit planner, figure inventory, source-data/statistics inventory, Reporting Summary placeholder and answer bank, author declarations checklist, and readiness checklist.",
    ]
    memory["scope_rule"] = (
...
def _update_roadmap_memory() -> None:
    ...
    current["after_stage9_27_submission_package_assembly"] = (
        ...
        "references, code-for-review surface, cover-letter submission draft, final upload runbook, prior-art positioning matrix, validation breadth map, editor-objection response map, two-minute editor triage simulation, Nature Methods editorial-bar rescue audit, current Nature Methods policy preflight, reviewer/editor fit planner, source-data/statistics inventory, Reporting Summary placeholder and answer bank, author declarations checklist, and readiness checklist."
    )
