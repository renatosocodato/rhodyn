...
def check_release(root: Path = ROOT) -> dict[str, object]:
    ...
    if memory_path.exists():
        ...
        if 10 in stages:
            ...
            stage10_pitch_gate = root / "case_studies" / "stage10_manuscript_pitch" / "stage10_6_gate_report.json"
            if stage10_pitch_gate.exists():
                try:
                    pitch_payload = json.loads(stage10_pitch_gate.read_text(encoding="utf-8"))
                except json.JSONDecodeError as exc:
                    failures.append(f"Stage 10.6 manuscript-pitch report is not valid JSON: {exc}")
                else:
                    if pitch_payload.get("status") != "pass":
                        failures.append("Stage 10.6 manuscript-pitch report must pass")
                    gates = pitch_payload.get("gates", {})
                    if not isinstance(gates, dict) or not all(gates.values()):
                        failures.append("Stage 10.6 manuscript-pitch gates must all pass")
                    summary = pitch_payload.get("summary_metrics", {})
                    if not isinstance(summary, dict) or summary.get("abstract_word_count") != 200:
                        failures.append("Stage 10.6 abstract must remain the 200-word method-first version")
            ...
            stage10_red_team_gate = root / "case_studies" / "stage10_eic_red_team" / "stage10_8_gate_report.json"
            if stage10_red_team_gate.exists():
                try:
                    red_team_payload = json.loads(stage10_red_team_gate.read_text(encoding="utf-8"))
                except json.JSONDecodeError as exc:
                    failures.append(f"Stage 10.8 EIC red-team report is not valid JSON: {exc}")
                else:
                    if red_team_payload.get("status") != "pass":
                        failures.append("Stage 10.8 EIC red-team report must pass")
                    gates = red_team_payload.get("gates", {})
                    if not isinstance(gates, dict) or not all(gates.values()):
                        failures.append("Stage 10.8 EIC red-team gates must all pass")
                    summary = red_team_payload.get("summary_metrics", {})
                ...
                failures.append("Stage 10.8 EIC red-team report is missing")
            stage10_contact_gate = root / "case_studies" / "stage10_eic_contact_decision" / "stage10_9_gate_report.json"
            if stage10_contact_gate.exists():
                try:
                    contact_payload = json.loads(stage10_contact_gate.read_text(encoding="utf-8"))
                except json.JSONDecodeError as exc:
                    failures.append(f"Stage 10.9 EIC-contact decision report is not valid JSON: {exc}")
                else:
                    if contact_payload.get("status") != "pass":
                        failures.append("Stage 10.9 EIC-contact decision report must pass")
                    if contact_payload.get("selected_route") != "presubmission_query_author_review_required":
                        failures.append("Stage 10.9 must select presubmission-style contact with author review required")
                    if contact_payload.get("external_contact_status") != "not_sent":
                        failures.append("Stage 10.9 must not mark external contact as sent")
                    gates = contact_payload.get("gates", {})
                    if not isinstance(gates, dict) or not all(gates.values()):
                        failures.append("Stage 10.9 EIC-contact decision gates must all pass")
                    summary = contact_payload.get("summary_metrics", {})
                ...
                failures.append("Stage 10.9 EIC-contact decision report is missing")
            stage10_hardening_gate = root / "case_studies" / "stage10_recursive_hardening" / "stage10_10_gate_report.json"
            if stage10_hardening_gate.exists():
                try:
                    hardening_payload = json.loads(stage10_hardening_gate.read_text(encoding="utf-8"))
                except json.JSONDecodeError as exc:
                    failures.append(f"Stage 10.10 recursive-hardening report is not valid JSON: {exc}")
                else:
                    if hardening_payload.get("status") != "pass":
                        failures.append("Stage 10.10 recursive-hardening report must pass")
                    if hardening_payload.get("selected_route") != "presubmission_query_author_review_required":
                        failures.append("Stage 10.10 must preserve the presubmission route")
                    if hardening_payload.get("external_contact_status") != "not_sent":
                        failures.append("Stage 10.10 must preserve external contact as not sent")
                    gates = hardening_payload.get("gates", {})
                    if not isinstance(gates, dict) or not all(gates.values()):
                        failures.append("Stage 10.10 recursive-hardening gates must all pass")
                    summary = hardening_payload.get("summary_metrics", {})
                    ...
                    if not isinstance(summary, dict) or summary.get("phase_pass_count") != 10:
                        failures.append("Stage 10.10 must keep all ten phase gates passing")
                ...
                failures.append("Stage 10.10 recursive-hardening report is missing")
            stage10_readiness_gate = root / "case_studies" / "stage10_author_review_readiness" / "stage10_11_gate_report.json"
            if stage10_readiness_gate.exists():
                try:
                    readiness_payload = json.loads(stage10_readiness_gate.read_text(encoding="utf-8"))
                except json.JSONDecodeError as exc:
                    failures.append(f"Stage 10.11 author-review readiness report is not valid JSON: {exc}")
                else:
                    if readiness_payload.get("status") != "pass":
                        failures.append("Stage 10.11 author-review readiness report must pass")
                    if readiness_payload.get("selected_route") != "presubmission_query_author_review_required":
                        failures.append("Stage 10.11 must preserve the presubmission route")
                    if readiness_payload.get("external_contact_status") != "not_sent":
                        failures.append("Stage 10.11 must preserve external contact as not sent")
                    gates = readiness_payload.get("gates", {})
                    if not isinstance(gates, dict) or not all(gates.values()):
                        failures.append("Stage 10.11 author-review readiness gates must all pass")
                    summary = readiness_payload.get("summary_metrics", {})
            ...
            stage10_visual_gate = root / "case_studies" / "stage10_rendered_figure_visual_qc" / "stage10_14_gate_report.json"
            if stage10_visual_gate.exists():
                try:
                    visual_payload = json.loads(stage10_visual_gate.read_text(encoding="utf-8"))
                except json.JSONDecodeError as exc:
                    failures.append(f"Stage 10.14 rendered-figure visual-QA report is not valid JSON: {exc}")
                else:
                    if visual_payload.get("status") != "pass":
                        failures.append("Stage 10.14 rendered-figure visual-QA report must pass")
                    if visual_payload.get("external_contact_status") != "not_sent":
                        failures.append("Stage 10.14 must preserve external contact as not sent")
                    if visual_payload.get("parent_stage10_13_visual_status") != "failed_visual_review_recorded":
                        failures.append("Stage 10.14 must record the Stage 10.13 parent visual failure")
                    if visual_payload.get("review_render_status") != "pass":
                        failures.append("Stage 10.14 review renders must pass")
                    gates = visual_payload.get("gates", {})
                    if not isinstance(gates, dict) or not all(gates.values()):
                        failures.append("Stage 10.14 rendered-figure visual-QA gates must all pass")
                    summary = visual_payload.get("summary_metrics", {})
                    ...
                    if not isinstance(summary, dict) or summary.get("stage9_rendered_file_count") != 18:
                        failures.append("Stage 10.14 must preserve the eighteen Stage 9 rendered files")
                    rendered_files = visual_payload.get("review_rendered_files", [])
                    if not isinstance(rendered_files, list) or len(rendered_files) != 18:
            ...
            stage10_message_gate = root / "case_studies" / "stage10_message_integrity" / "stage10_17_gate_report.json"
            if stage10_message_gate.exists():
                try:
                    message_payload = json.loads(stage10_message_gate.read_text(encoding="utf-8"))
                except json.JSONDecodeError as exc:
                    failures.append(f"Stage 10.17 message-integrity report is not valid JSON: {exc}")
                else:
                    if message_payload.get("status") != "pass":
                        failures.append("Stage 10.17 message-integrity report must pass")
                    if message_payload.get("external_contact_status") != "not_sent":
                        failures.append("Stage 10.17 must preserve external contact as not sent")
                    gates = message_payload.get("gates", {})
                    if not isinstance(gates, dict) or not all(gates.values()):
                        failures.append("Stage 10.17 message-integrity gates must all pass")
                    summary = message_payload.get("summary_metrics", {})
                ...
                failures.append("Stage 10.17 message-integrity report is missing")
            stage10_approval_gate = root / "case_studies" / "stage10_author_approval_dossier" / "stage10_18_gate_report.json"
            if stage10_approval_gate.exists():
                try:
                    approval_payload = json.loads(stage10_approval_gate.read_text(encoding="utf-8"))
                except json.JSONDecodeError as exc:
                    failures.append(f"Stage 10.18 author-approval report is not valid JSON: {exc}")
                else:
                    if approval_payload.get("status") != "pass":
                        failures.append("Stage 10.18 author-approval report must pass")
                    if approval_payload.get("external_contact_status") != "not_sent":
                        failures.append("Stage 10.18 must preserve external contact as not sent")
                    if approval_payload.get("recommendation") != "presubmission_query_after_corresponding_author_approval":
                        failures.append("Stage 10.18 must retain presubmission after corresponding-author approval")
                    gates = approval_payload.get("gates", {})
                    if not isinstance(gates, dict) or not all(gates.values()):
                        failures.append("Stage 10.18 author-approval gates must all pass")
                    summary = approval_payload.get("summary_metrics", {})
                ...
                failures.append("Stage 10.18 author-approval report is missing")
            subphase_status = {
                entry.get("id"): entry.get("status")
                for entry in stage10.get("subphases", [])
            ...
            stage10_closeout_gate = root / "case_studies" / "stage10_full_chain_closeout" / "stage10_19_gate_report.json"
            if stage10_closeout_gate.exists():
                try:
                    closeout_payload = json.loads(stage10_closeout_gate.read_text(encoding="utf-8"))
                except json.JSONDecodeError as exc:
