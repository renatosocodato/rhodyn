...
STAGE9_27_RUNNER_PATH = ROOT / "scripts" / "run_stage9_27_submission_package_assembly.py"
STAGE9_28_RUNNER_PATH = ROOT / "scripts" / "run_stage9_28_pi_review_auto_revision.py"
STAGE9_29_RUNNER_PATH = ROOT / "scripts" / "run_stage9_29_closure_assembly.py"
...
def check_roadmap_memory(root: Path = ROOT) -> dict[str, object]:
    ...
    stage9_27_runner_path = root / STAGE9_27_RUNNER_PATH.relative_to(ROOT)
    stage9_28_runner_path = root / STAGE9_28_RUNNER_PATH.relative_to(ROOT)
    stage9_29_runner_path = root / STAGE9_29_RUNNER_PATH.relative_to(ROOT)
    ...
    stage9_27_gate_path = root / STAGE9_27_GATE_PATH.relative_to(ROOT)
    stage9_28_gate_path = root / STAGE9_28_GATE_PATH.relative_to(ROOT)
    stage9_29_gate_path = root / STAGE9_29_GATE_PATH.relative_to(ROOT)
    ...
    expected_status = {
        ...
        10: "stage10_21_complete_figure_recipe_diversification",
    }
    ...
    if stage10:
        ...
        heldout_report_path = root / "case_studies" / "stage10_heldout_validation" / "stage10_4_gate_report.json"
        if heldout_report_path.exists():
            ...
            summary = heldout_report.get("summary_metrics", {})
            ...
            gates = heldout_report.get("gates", {})
            if not isinstance(gates, dict) or not all(gates.values()):
                failures.append("Stage 10.4 held-out gates must all pass")
        else:
            failures.append("missing Stage 10.4 held-out report")
        if subphase_status.get("10.4") != "complete_heldout_validation":
            failures.append("Stage 10.4 must be complete_heldout_validation")
        if subphase_status.get("10.5") != "complete_method_first_figure_architecture":
            failures.append("Stage 10.5 must be complete_method_first_figure_architecture")
        figure_report_path = root / "case_studies" / "stage10_figure_architecture" / "stage10_5_gate_report.json"
        if figure_report_path.exists():
            figure_report = json.loads(figure_report_path.read_text(encoding="utf-8"))
            if figure_report.get("status") != "pass":
                failures.append("Stage 10.5 figure-architecture report must pass")
            summary = figure_report.get("summary_metrics", {})
            if not isinstance(summary, dict) or summary.get("figure_count") != 6:
                failures.append("Stage 10.5 must define six main figures")
            if not isinstance(summary, dict) or summary.get("panel_count", 0) < 30:
                failures.append("Stage 10.5 must define at least 30 panels")
            gates = figure_report.get("gates", {})
            if not isinstance(gates, dict) or not all(gates.values()):
        ...
        if subphase_status.get("10.7") != "complete_benchmark_release_candidate":
            failures.append("Stage 10.7 must be complete_benchmark_release_candidate")
        release_report_path = root / "case_studies" / "stage10_release_candidate" / "stage10_7_gate_report.json"
        if release_report_path.exists():
            ...
            summary = release_report.get("summary_metrics", {})
        ...
        if subphase_status.get("10.12") != "complete_optional_strengthening_triage":
            failures.append("Stage 10.12 must be complete_optional_strengthening_triage")
        strengthening_report_path = root / "case_studies" / "stage10_optional_strengthening" / "stage10_12_gate_report.json"
        if strengthening_report_path.exists():
            strengthening_report = json.loads(strengthening_report_path.read_text(encoding="utf-8"))
            if strengthening_report.get("status") != "pass":
                failures.append("Stage 10.12 optional-strengthening triage report must pass")
            if strengthening_report.get("external_contact_status") != "not_sent":
                failures.append("Stage 10.12 must preserve external contact as not sent")
            if strengthening_report.get("recommended_local_next_step") != "render_stage10_method_figures":
                failures.append("Stage 10.12 must select Stage 10 figure rendering as the next local hardening step")
            gates = strengthening_report.get("gates", {})
            if not isinstance(gates, dict) or not all(gates.values()):
                failures.append("Stage 10.12 optional-strengthening gates must all pass")
            summary = strengthening_report.get("summary_metrics", {})
            if not isinstance(summary, dict) or summary.get("figure_count") != 6:
            ...
            failures.append("missing Stage 10.12 optional-strengthening triage report")
        if subphase_status.get("10.13") != "complete_rendered_method_figures":
            failures.append("Stage 10.13 must be complete_rendered_method_figures")
        rendered_gate_path = root / "case_studies" / "stage10_rendered_figures" / "stage10_13_gate_report.json"
        if rendered_gate_path.exists():
            rendered_gate = json.loads(rendered_gate_path.read_text(encoding="utf-8"))
            if rendered_gate.get("status") != "pass":
                failures.append("Stage 10.13 rendered-method-figure report must pass")
            if rendered_gate.get("external_contact_status") != "not_sent":
                failures.append("Stage 10.13 must preserve external contact as not sent")
            gates = rendered_gate.get("gates", {})
            if not isinstance(gates, dict) or not all(gates.values()):
                failures.append("Stage 10.13 rendered-method-figure gates must all pass")
            summary = rendered_gate.get("summary_metrics", {})
            if not isinstance(summary, dict) or summary.get("figure_count") != 6:
            ...
            panelforge = rendered_gate.get("panelforge", {})
            ...
            rendered_files = rendered_gate.get("rendered_files", [])
        ...
        if recipe_report_path.exists():
            ...
            failures.append("missing Stage 10.21 figure recipe diversification report")
    ...
    if stage9_status.get("9.22") != "complete_statistical_language_audit_bound":
        failures.append("Stage 9.22 must be complete_statistical_language_audit_bound")
    if stage9_status.get("9.23") != "complete_figure_legend_caption_audit_bound":
    ...
    if stage9_status.get("9.27") != "complete_submission_package_assembled":
        failures.append("Stage 9.27 must be complete_submission_package_assembled")
    if stage9_status.get("9.28") != "complete_pi_review_packet":
        failures.append("Stage 9.28 must be complete_pi_review_packet")
    if stage9_status.get("9.29") != "complete_stage9_closed_version_bound":
        failures.append("Stage 9.29 must be complete_stage9_closed_version_bound")
    for entry in stage9_substages:
        if isinstance(entry, dict) and entry.get("id") not in {"9.-1", "9.0", "9.1", "9.2", "9.3", "9.4", "9.5", "9.6", "9.6b", "9.7", "9.8", "9.9", "9.10", "9.11", "9.12", "9.13", "9.14", "9.15", "9.16", "9.17", "9.18", "9.19", "9.20", "9.21", "9.22", "9.23", "9.24", "9.25", "9.25b", "9.26", "9.27", "9.28", "9.29"} and entry.get("status") != "not_started":
    ...
    required_roadmap_phrases = [
        ...
        "Stage 9.27 Submission package assembly has been completed",
        "Stage 9.28 Final human PI review packet has been completed",
        "Stage 9.29 Roadmap closure and version binding has been completed",
    ...
    stage9_docs = [
        (stage9_plan_path, "Stage 9 manuscript assembly plan", ["9.-1", "9.0", "9.1", "9.2", "9.3", "9.4", "9.5", "9.6", "9.6b", "9.7", "9.8", "9.9", "9.10", "9.11", "9.12", "9.13", "9.14", "9.15", "9.16", "9.17", "9.18", "9.19", "9.20", "9.21", "9.22", "9.23", "9.24", "9.25", "9.25b", "9.26", "9.27", "9.28", "9.29", "PanelForge", "evidence lock", "Results drafting", "Introduction literature binding", "Discussion drafting", "Methods architecture", "Methods prose", "data availability", "code availability", "Supplementary Methods", "supplementary table/source-data binding", "reference library", "Cross-document consistency", "statistical", "figure legends", "Editorial polish pass I", "Editorial polish pass II", "reader-surface hygiene", "internal peer review", "submission package", "PI review", "closure"]),
        (stage9_memory_path, "Stage 9 execution memory", ["stage9_29_closed_version_bound", "9.-1", "9.0", "9.1", "9.2", "9.3", "9.4", "9.5", "9.6", "9.6b", "9.7", "9.8", "9.9", "9.10", "9.11", "9.12", "9.13", "9.14", "9.15", "9.16", "9.17", "9.18", "9.19", "9.20", "9.21", "9.22", "9.23", "9.24", "9.25", "9.25b", "9.26", "9.27", "9.28", "9.29", "figure_engine_clone_started", "reference_library_started", "cross_document_consistency_started", "statistical_language_audit_started", "figure_legends_started", "editorial_polish_pass_1_started", "editorial_polish_pass_2_started", "reader_surface_hygiene_started", "internal_peer_review_started", "submission_package_started", "pi_review_started", "stage9_closed"]),
        ...
        (stage9_27_runner_path, "Stage 9.27 submission package runner", ["main_text_for_submission.md", "submission_readiness_checklist.md", "code_for_review.md", "package_consistency_audit_passed", "Stage 9.28"]),
        (stage9_28_runner_path, "Stage 9.28 PI review runner", ["pi_review_packet.md", "pi_review_action_matrix.csv", "pi_review_revision_log.md", "pi_review_literature_calibration.md", "Stage 9.29"]),
        (stage9_29_runner_path, "Stage 9.29 closure runner", ["stage9_completion_report.md", "stage9_closure_version_binding.json", "pi_review_action_decisions.csv", "complete_stage9_closed_version_bound"]),
    ]
    ...
    if not stage9_22_gate_path.exists():
        failures.append("missing manuscript/nature_methods/gate_verdicts/9.22.json")
    else:
        ...
        if stage9_22_gate.get("substage") != "9.22":
            failures.append("Stage 9.22 statistical-language gate must remain bound to substage 9.22")
        ...
        stat_audit_path = root / "manuscript" / "nature_methods" / "audits" / "statistical_language_audit.md"
        live_diff_path = root / "manuscript" / "nature_methods" / "audits" / "live_numbers_diff.csv"
        statistic_ledger_path = root / "manuscript" / "nature_methods" / "ledgers" / "statistic_ledger.csv"
        figure_ledger_path = root / "manuscript" / "nature_methods" / "ledgers" / "figure_to_claim_to_artifact.csv"
        for output_path in [stat_audit_path, live_diff_path, statistic_ledger_path, figure_ledger_path]:
            if not output_path.exists():
                failures.append(f"missing Stage 9.22 output: {output_path.relative_to(root)}")
        if stat_audit_path.exists():
            audit_text = stat_audit_path.read_text(encoding="utf-8")
            for phrase in ["The live-number audit passed", "STAT-0018", "bounded-coupling language remains scoped"]:
                if phrase not in audit_text:
                    failures.append(f"Stage 9.22 statistical-language audit missing phrase: {phrase}")
        ...
        if figure_ledger_path.exists() and "pending_stage9.22" in figure_ledger_path.read_text(encoding="utf-8"):
            failures.append("Stage 9.22 figure ledger must not retain pending_stage9.22")

    if not stage9_23_gate_path.exists():
        failures.append("missing manuscript/nature_methods/gate_verdicts/9.23.json")
    else:
        ...
        figure_legends_path = root / "manuscript" / "nature_methods" / "figures" / "figure_legends.md"
        figure_legend_audit_path = root / "manuscript" / "nature_methods" / "audits" / "figure_legend_audit.md"
        for output_path in [figure_legends_path, figure_legend_audit_path]:
            if not output_path.exists():
                failures.append(f"missing Stage 9.23 output: {output_path.relative_to(root)}")
        if figure_legends_path.exists():
            legend_text = figure_legends_path.read_text(encoding="utf-8")
            for phrase in [
            ...
            leakage_pattern = "FIG-|SFIG-|STBL-|SUPP-|STAT-|ART-|CLM-|PARA-|PanelForge|panelforge|render_path|source_paths|/" + "Users/|/" + "Volumes/"
            if any(token in legend_text for token in leakage_pattern.split("|")):
        ...
        if figure_legend_audit_path.exists():
            audit_text = figure_legend_audit_path.read_text(encoding="utf-8")
            for phrase in [
    ...
    if not stage9_24_gate_path.exists():
        failures.append("missing manuscript/nature_methods/gate_verdicts/9.24.json")
    else:
        ...
        for check_name in [
            ...
            if stage9_24_checks.get(check_name) is not True:
                failures.append(f"Stage 9.24 editorial-polish gate check must pass: {check_name}")
        editorial_audit_path = root / "manuscript" / "nature_methods" / "audits" / "editorial_pass_1.md"
    ...
    if not stage9_25b_gate_path.exists():
        failures.append("missing manuscript/nature_methods/gate_verdicts/9.25b.json")
    else:
        ...
        for check_name in [
            ...
            if stage9_25b_checks.get(check_name) is not True:
                failures.append(f"Stage 9.25b reader-surface hygiene gate check must pass: {check_name}")
        hygiene_audit_path = root / "manuscript" / "nature_methods" / "audits" / "reader_surface_hygiene_report.md"
        if not hygiene_audit_path.exists():
            ...
            audit_text = hygiene_audit_path.read_text(encoding="utf-8")
            for phrase in [
    ...
    if not stage9_27_gate_path.exists():
        failures.append("missing manuscript/nature_methods/gate_verdicts/9.27.json")
    else:
        ...
        for check_name in [
            ...
            if stage9_27_checks.get(check_name) is not True:
                failures.append(f"Stage 9.27 submission package gate check must pass: {check_name}")
        for rel in [
    ...
    if not stage9_28_gate_path.exists():
        failures.append("missing manuscript/nature_methods/gate_verdicts/9.28.json")
    else:
        ...
        for check_name in [
            ...
            if stage9_28_checks.get(check_name) is not True:
                failures.append(f"Stage 9.28 PI-review gate check must pass: {check_name}")
        for rel in [
    ...
    if not stage9_29_gate_path.exists():
        failures.append("missing manuscript/nature_methods/gate_verdicts/9.29.json")
    else:
        ...
        for check_name in [
            ...
            if stage9_29_checks.get(check_name) is not True:
                failures.append(f"Stage 9.29 closure gate check must pass: {check_name}")
        for rel in [
    ...
    warnings.append("Stage 9.29 closure is registered; journal upload actions remain human submission tasks")
