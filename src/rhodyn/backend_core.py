...
def _parameter_provenance(
    ...
    *,
    ...
    return {
        ...
        "interpretation_boundary": "Parameter provenance records the declared analysis choices for this run. It does not change the submitted measurements or upgrade derived summaries into direct biological mechanisms.",
    }


def _parameter_provenance_markdown(provenance: dict[str, Any]) -> str:
    submitted = provenance.get("submitted_parameters", {})
    effective = provenance.get("effective_parameters", {})
    lines = [
        "# RhoDyn parameter provenance",
        "",
        f"Operation: `{provenance.get('operation', '')}`",
        f"Status: `{provenance.get('status', '')}`",
        f"Job ID: `{provenance.get('job_id', '')}`",
        ...
        "",
        provenance.get("interpretation_boundary", ""),
        "",
    ]
    return "\n".join(lines)


def _readme_text(operation: str, result: dict[str, Any], provenance: dict[str, Any]) -> str:
    job = result.get("job", {})
    status = result.get("status", "")
    lines = [
        ...
        "explicit parameter provenance, a compact result table, a Markdown report,",
        "and payload SHA-256 checksums.",
...
def build_analysis_bundle(
    ...
    *,
    ...
    files: dict[str, bytes] = {
        ...
        "parameter_provenance.json": json.dumps(provenance, indent=2, sort_keys=True).encode("utf-8") + b"\n",
        "parameter_provenance.md": _parameter_provenance_markdown(provenance).encode("utf-8"),
