...
Stage 10.5 has completed the method-first figure architecture, and Stage 10.6
has completed the method-first manuscript and pitch transformation. Stage 10.7
benchmark-ready release-candidate assembly has not started.
...
| Stage 10. Nature Methods EIC rescue and methodological elevation | Stage 10.6 manuscript-pitch transformation complete; Stage 10.7 benchmark-ready release candidate not started. | This is a post-closure evidence-elevation program, not another Stage 9 polish pass. It exists to lower the risk that RhoDyn is read as useful workflow/software integration rather than as a competitive Nature Methods-level method. Stage 10 has now added named baseline and named-tool benchmarking, broader public biological breadth, a no-retuning held-out validation route, a method-first display spine, and method-first manuscript/pitch surfaces that place the method object, named baselines, biological breadth, and held-out validation before software reproducibility. |
