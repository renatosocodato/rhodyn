...
demonstrations, and Stage 10.4 has completed a sealed held-out validation
route. Stage 10.5 method-first figure architecture has not started.
...
| Stage 10. Nature Methods EIC rescue and methodological elevation | Stage 10.4 sealed held-out validation complete; Stage 10.5 method-first figure architecture not started. | This is a post-closure evidence-elevation program, not another Stage 9 polish pass. It exists to lower the risk that RhoDyn is read as useful workflow/software integration rather than as a competitive Nature Methods-level method. Stage 10 has now added named baseline and named-tool benchmarking, broader public biological breadth, and a no-retuning held-out validation route; it still needs a method-first figure rebuild and adversarial EIC simulation before any second EIC contact. |
