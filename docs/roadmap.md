...
and Stage 10.5 has completed the method-first figure architecture. Stage 10.6
manuscript-pitch transformation has not started.
...
| Stage 10. Nature Methods EIC rescue and methodological elevation | Stage 10.5 method-first figure architecture complete; Stage 10.6 manuscript-pitch transformation not started. | This is a post-closure evidence-elevation program, not another Stage 9 polish pass. It exists to lower the risk that RhoDyn is read as useful workflow/software integration rather than as a competitive Nature Methods-level method. Stage 10 has now added named baseline and named-tool benchmarking, broader public biological breadth, a no-retuning held-out validation route, and a method-first display spine that places the method object, named baselines, biological breadth, and held-out validation before software reproducibility. |
