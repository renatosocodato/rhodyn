...
Recursive hardening after 7.8 is allowed only as a release-consistency check over completed Stage 7 outputs. It may verify archive coverage, checksums, bundle provenance, validation status, and roadmap boundaries. It may also verify the Stage 9 scaffold-only contract layer once serialized, including the future PanelForge handoff, but it must not start Stage 9 manuscript production, evidence intake, citation resolution, PanelForge figure rendering, drafting, or add a new scientific claim.
...
Stage 9 scaffold status. Serialized but not started as manuscript production. The current Stage 9 state is limited to `docs/stage9_manuscript_assembly_plan.md`, `docs/stage9_execution_memory.json`, the `manuscript/nature_methods/contracts/` schema layer, the placeholder `manuscript/nature_methods/figures/figures.manifest.yaml`, the reserved PanelForge placeholder under `tools/panelforge-figures/`, and `manuscript/nature_methods/gate_verdicts/9.-1.json`. It records the Nature Methods manuscript-assembly contract but does not start evidence intake, citation resolution, PanelForge rendering, drafting, figure assembly, response writing, or submission packaging.
