...
Recursive hardening after 7.8 is allowed only as a release-consistency check over completed Stage 7 outputs. It may verify archive coverage, checksums, bundle provenance, validation status, and roadmap boundaries. It may also verify the Stage 9 contract layer and Stage 9.0 evidence lock, including the future PanelForge handoff, but it must not start Stage 9 manuscript production, citation resolution, PanelForge figure rendering, drafting, or add a new scientific claim.
...
Stage 9 scaffold status. Serialized and followed by Stage 9.0 evidence lock. The current Stage 9 state includes `docs/stage9_manuscript_assembly_plan.md`, `docs/stage9_execution_memory.json`, the `manuscript/nature_methods/contracts/` schema layer, the placeholder `manuscript/nature_methods/figures/figures.manifest.yaml`, the reserved PanelForge placeholder under `tools/panelforge-figures/`, `manuscript/nature_methods/gate_verdicts/9.-1.json`, `manuscript/nature_methods/gate_verdicts/9.0.json`, and the three Stage 9.0 evidence-lock ledgers. It records the Nature Methods manuscript-assembly contract and locks the Stage 7.8 evidence package, but it does not start citation resolution, PanelForge rendering, drafting, figure assembly, response writing, or submission packaging.
